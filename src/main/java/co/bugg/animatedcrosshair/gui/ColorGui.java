...
public class ColorGui extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.add(new GuiSlider(responder, buttonId, width / 2 - sliderWidth / 2, height / 2 - sliderHeight / 2 + (sliderHeight + sliderMargin) * (buttonId - 2), new ChatComponentTranslation("animatedcrosshair.color.red").getUnformattedText(), 0F, 255F, properties.colorModifier.getRed(), formatHelper));
        buttonId++;
        buttonList.add(new GuiSlider(responder, buttonId, width / 2 - sliderWidth / 2, height / 2 - sliderHeight / 2 + (sliderHeight + sliderMargin) * (buttonId - 2), new ChatComponentTranslation("animatedcrosshair.color.green").getUnformattedText(), 0F, 255F, properties.colorModifier.getGreen(), formatHelper));
        buttonId++;
        buttonList.add(new GuiSlider(responder, buttonId, width / 2 - sliderWidth / 2, height / 2 - sliderHeight / 2 + (sliderHeight + sliderMargin) * (buttonId - 2), new ChatComponentTranslation("animatedcrosshair.color.blue").getUnformattedText(), 0F, 255F, properties.colorModifier.getBlue(), formatHelper));
        buttonId++;
        // TODO
...
//        buttonId++;
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if(button.displayString.equalsIgnoreCase(new ChatComponentTranslation("animatedcrosshair.config.save").getUnformattedText())) {
        ...
        } else if(button.displayString.contains(new ChatComponentTranslation("animatedcrosshair.color.chroma").getUnformattedText())) {
            // Swap the "Chroma Color" value
            properties.chromaColor = !properties.chromaColor;
            button.displayString = new ChatComponentTranslation("animatedcrosshair.color.chroma").getUnformattedText() + ": " + (properties.chromaColor ? new ChatComponentTranslation("animatedcrosshair.config.enabled").getUnformattedText() : new ChatComponentTranslation("animatedcrosshair.config.disabled").getUnformattedText());
            ...
            button.displayString = new ChatComponentTranslation("animatedcrosshair.properties.negativecolor").getUnformattedText() + ": " + (properties.negativeColor ? new ChatComponentTranslation("animatedcrosshair.config.enabled").getUnformattedText() : new ChatComponentTranslation("animatedcrosshair.config.disabled").getUnformattedText());
        }
    }
    ...
    public class ColorGuiResponder implements GuiPageButtonList.GuiResponder {
        ...
        public void onTick(int id, float value) {
            if(buttonList.get(id).displayString.contains(new ChatComponentTranslation("animatedcrosshair.color.red").getUnformattedText())) {
                properties.colorModifier = new Color((int) value, properties.colorModifier.getGreen(), properties.colorModifier.getBlue());
            } else if(buttonList.get(id).displayString.contains(new ChatComponentTranslation("animatedcrosshair.color.green").getUnformattedText())) {
                properties.colorModifier = new Color(properties.colorModifier.getRed(), (int) value, properties.colorModifier.getBlue());
            } else if(buttonList.get(id).displayString.contains(new ChatComponentTranslation("animatedcrosshair.color.blue").getUnformattedText())) {
